...
module.exports = async (pluginConfig, context) => {
  ...
  logger.log("Using repository '%s' owned by '%s'", repo, owner);
  ...
  logger.log("Using repository root path '%s'", absoluteRepoPath);
  ...
  //       commitSha = sha;
  //     } catch (error) {
  //       logger.log("Catched an error '%s'", error.message);
  //       if (error.status === 404) {
  //         // ignore error
  //       }
  //     }
  ...
  //     logger.log("Upload file '%s'", commitSha);
  //     logger.log("Upload file '%s'", ref);
